...
Although sociological studies \cite{35626} show that the probability of edge formation tends to decrease as a function of network distance,
little is known about the structural proximity among an incoming node's connections before the incoming node forms edges.
As shown in~\Cref{fig:locality}, high structural proximity between an incoming (blue) node's (red) connections in the \texttt{Hep-PH} dataset
hints at edge formation processes biased towards nodes in the same local neighborhood.
\begin{figure}[b]
 \vspace{-4pt}
 \centering
 \includegraphics[width=\columnwidth]{locality_v3}
 \caption{
    Proximity-biased edge formation. The diagram and proximity distributions collectively
    indicate how edge formation in real-world networks are biased towards
    structurally proximate nodes in the same locality.
 }
 \label{fig:locality}
 \vspace{-10pt}
\end{figure}

Due to the unavailability of navigational data such as the sequence in which
individuals form edges, we rely on coarser temporal data---network snapshots and
...
The second subplot in~\Cref{fig:locality} compares the proximity statistic
distribution of the \texttt{Hep-PH} dataset to uniform and configuration null
models, which randomly rewire all edges in \texttt{Hep-PH} while preserving the
out-degree distribution and both degree distributions respectively. Although the
presence of hubs in the configuration null model significantly decreases
pairwise distance between nodes' connections, it does not explain why a large
fraction of \texttt{Hep-PH} connections are either connected directly or via an
intermediate node. The disparity between the observed and null distributions
suggests that structural proximity between connections is intrinsic to edge
