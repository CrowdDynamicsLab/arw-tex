...
We investigate the effect of structural proximity on edge formation in
real-world networks. While prior work~\cite{35626} shows that the probability of
...
% We investigate the effect of structural proximity on edge formation in
% real-world networks. Although sociological studies \cite{35626} show that the
% probability of edge formation tends to decrease as a function of network
% distance, little is known about the structural proximity among an incoming
% node's connections before the incoming node forms edges. As shown
% in~\Cref{fig:locality}, high structural proximity between an incoming (blue)
% node's (red) connections in the \texttt{Hep-PH} dataset hints at edge formation
% processes biased towards nodes in the same local neighborhood.
