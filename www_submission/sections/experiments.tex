...
to each network dataset $G$ as described in~\Cref{sub:Experimental Setup}.
Thereafter, we compare the structural properties of network dataset $G$ and network $\hat{G}$
generated by the fitted model using evaluation metrics introduced in~\Cref{sub:Experimental Setup}. We average out
