...
\texttt{ARW} parameters intuitively modulate global structural properties:
...
\begin{figure}
 % \vspace{-10pt}
 \centering
 \includegraphics[width=1.05\columnwidth]{inter22}
 \caption{
...
\end{figure}
 ...
 In~\Cref{fig:inter2}, we use contour plots to visualize how $(\pdiff, \psame)$
and $(\plink, \pjump)$ alter attribute assortativity and average path length.
As shown in the left subplot, $\psame-\pdiff$ tunes the extent to which
...
thereby allowing edge formation to nodes structurally distant from their seed
nodes. Additionally, increasing $\pout$ results in greater number of hubs,
which in turn act as intermediate nodes to connect nodes via short path
lengths.

To summarize,
\texttt{ARW} incorporates multiple sociological phenomena at the local level
