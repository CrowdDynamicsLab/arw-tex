...
\end{ph}
The attribute-biased link formation parameters $\psame$ and $\pdiff$ modulate
...
\texttt{ARW} parameters, we explore the parameter space of the model.
\texttt{ARW} uses $(\plink, \pjump, \pout)$
and $(\psame, \pdiff, \pjump, \pout)$ to generate networks with and without attribute data. respectively.
We use both parameterizations of the model to examine properties of structure
and attributes.
We analyze structural properties using the former and
attribute assortativity using the latter.
