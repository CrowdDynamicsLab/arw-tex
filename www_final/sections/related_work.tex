...
with one or more common neighbors, as shown in~\Cref{sec:Experiments}, it does not
...
% a new node $u$ links to an existing node $v$
...
% power law degree distributions and small diameter
...
% model the distribution over the local clustering coefficient or the
% relationship between in-degree and clustering.
% models
% based on preferential attachment and triangle closing do not preserve the local
% clustering of low degree nodes.

% preserve assortative
...
%  disregard the effect of homophily and do not model attribute mixing
% patterns.
